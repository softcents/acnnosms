                                        ...
                                        <div class="row">
                                            <div class="col-sm-6 mt-2">
                                                ...
                                                <div class="gpt-up-down-arrow position-relative">
                                                    <select name="campaign_id" required="" class="form-control select-dropdown">
                                                        <option value="">{{ __('Select Recipients') }}</option>
                                                        @foreach ($campaigns as $campaign)
                                                        <option @selected($campaign->id == $sms->campaign_id) value="{{ $campaign->id }}">{{ $campaign->name }}</option>
                                                        @endforeach
                                                    </select>
                                                    <span></span>
                                                </div>
                                            ...
                                            <div class="col-sm-6 mt-2">
                                                ...
                                                <div class="gpt-up-down-arrow position-relative">
                                                    <select name="senderid_id" required="" class="form-control select-dropdown">
                                                        ...
                                                        <option @selected($senderid->id == $sms->senderid_id) value="{{ $senderid->id }}">{{ $senderid->sender }}</option>
                                                        @endforeach
                                                    </select>
                                                    <span></span>
                                                </div>
                                            ...
                                            <div class="col-sm-6 mt-2">
                                                ...
                                                <div class="gpt-up-down-arrow position-relative">
                                                    <select name="schedule_type" required="" class="form-control select-dropdown timer-option">
                                                        <option @selected(!$sms->schedule) value="sendnow">{{ __('Send Now') }}</option>
                                                        <option @selected($sms->schedule) value="schedule">{{ __('Schedule') }}</option>
                                                    </select>
                                                    <span></span>
                                                </div>
                                            </div>

                                            <div class="col-lg-6 mt-2 d-none schedule-sms">
                                                <label>{{ __('Schedule SMS') }}</label>
                                                <input type="datetime-local" name="schedule" value="{{ $sms->schedule }}" required class="form-control">
                                            </div>

                                            <div class="col-sm-12 mt-2">
                                                <label>{{ __('Message Content') }}</label>
                                                <textarea name="contents" id="contents" class="form-control" placeholder="{{ __('Enter your message content here') }}.">{{ $sms->contents }}</textarea>
                                                <div id="the-count" class="pull-right mt-1">
                                                    <span id="current">{{ __('0 Characters') }} </span> <span class="text-primary fw-bold
                                                    ">|</span>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('SMS Templates') }}</h5>
